...
{
    /// <summary>
    /// OutlineThicknessのアニメーション
    /// </summary>
    [Serializable]
    public class TMPM_OutlineThicknessAnimation : MatAnimationBase
    {
        /// <summary>
        /// マテリアルのパラメータID
        /// </summary>
        private int matParamId = Shader.PropertyToID("_OutlineWidth");
        ...
        protected override Sequence CreateSequence()
        {
            ...
            Tween tw;
            //アニメーション設定 + From設定
            if(additional.isFrom) tw = mat.DOFloat(afterThickness + nowFloat, matParamId, duration).From();
            else tw = mat.DOFloat(afterThickness + nowFloat, matParamId, duration);
