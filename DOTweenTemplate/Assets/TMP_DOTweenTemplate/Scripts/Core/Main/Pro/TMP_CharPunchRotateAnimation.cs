...
namespace TMP_DOTweenTemplate.Core.Main.Pro
{
    ...
    /// </summary>
    [Serializable]
    public class TMP_CharPunchRotateAnimation : CharAnimationBase
    {
        /// <summary>
        /// パンチの角度
        /// </summary>
        ...
        protected override Tween CreateTween(int index)
        {
            ...
            //アニメーション設定 + From設定  
            if(additional.isFrom) tw = tmp_Animator.DOPunchCharRotation(index, punchAngles + nowCharOffset, duration, vibrate, elasticity).From();
            else tw = tmp_Animator.DOPunchCharRotation(index, punchAngles + nowCharOffset, duration, vibrate, elasticity);
