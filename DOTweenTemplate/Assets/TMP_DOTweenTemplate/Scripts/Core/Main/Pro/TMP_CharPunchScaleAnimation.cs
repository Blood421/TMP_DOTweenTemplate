...
namespace TMP_DOTweenTemplate.Core.Main.Pro
{
    ...
    /// </summary>
    [Serializable]
    public class TMP_CharPunchScaleAnimation : CharAnimationBase
    {
        /// <summary>
        /// パンチの拡縮
        /// </summary>
        ...
        protected override Tween CreateTween(int index)
        {
            ...
            //アニメーション設定 + From設定   
            if(additional.isFrom) tw = tmp_Animator.DOPunchCharScale(index, punchScale + nowCharOffset, duration, vibrate, elasticity).From();
            else tw = tmp_Animator.DOPunchCharScale(index, punchScale + nowCharOffset, duration, vibrate, elasticity);
