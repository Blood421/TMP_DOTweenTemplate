...
namespace TMP_DOTweenTemplate.Core.Main.Pro
{
    ...
    /// </summary>
    [Serializable]
    public class TMP_CharShakeScaleAnimation : CharAnimationBase
    {
        /// <summary>
        /// シェイク拡縮
        /// </summary>
        ...
        protected override Tween CreateTween(int index)
        {
            ...
            //アニメーション設定 + From設定  
            if(additional.isFrom) tw = tmp_Animator.DOShakeCharScale(index, duration, punchScale + nowCharOffset, vibrate, randomness, fadeOut).From();
            else tw = tmp_Animator.DOShakeCharScale(index, duration, punchScale + nowCharOffset, vibrate, randomness, fadeOut);
